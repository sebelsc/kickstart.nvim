...
local workspace = vim.fn.stdpath 'data' ..
    '/jdtls/' .. vim.fn.fnamemodify(root, ':h:t') .. '/' .. vim.fn.fnamemodify(root, ':t')

-- ── Bundles: DAP (java-debug) + JUnit (vscode-java-test) ─────────────────
local bundles = vim.fn.glob(mason .. '/java-debug-adapter/extension/server/com.microsoft.java.debug.plugin-*.jar', true,
    true)
...
local config = {
    ...
    settings = {
        java = {
            ...
            configuration = {
                ...
                runtimes = {
                    { name = 'JavaSE-25', path = vim.fn.expand '/Library/Java/JavaVirtualMachines/temurin-25.jdk/Contents/Home', default = true },
                },
            },
    ...
    on_attach = function(client, bufnr)
        ...
        -- Gradle terminal tasks (Java-only, under <leader>t to extend the test group)
        local function gradle(task)
            Snacks.terminal.open('./gradlew ' .. task, {
                cwd = vim.fn.getcwd(),
                win = { style = 'terminal', height = 0.4, width = 0.9 },
            })
        end
        map('<leader>tG', function() gradle('test') end, 'gradlew test')
        map('<leader>tI', function() gradle('test --info') end, 'gradlew test --info')
        map('<leader>tC', function()
            local fqn = vim.fn.expand('%:p'):match('java/(.-)%.java$')
            if fqn then
                gradle("test --tests '" .. fqn:gsub('/', '.') .. "'")
            else
                vim.notify('Cannot resolve class from path', vim.log.levels.WARN)
            end
        end, 'gradlew test <class>')
        map('<leader>tT', function()
            vim.ui.input({ prompt = 'Gradle task: ', default = 'test' }, function(task)
                if task and task ~= '' then gradle(task) end
            end)
        end, 'gradlew <task>')
